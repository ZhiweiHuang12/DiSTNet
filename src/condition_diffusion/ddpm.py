...
class DDPM(nn.Module):
    ...
    def sample(self, n_samples: int, size: Tuple[int], classes: List[int], w: float,cond_sample=[]):
        self.eval()
        ...
        with torch.no_grad():
            ...
            if len(cond_sample)>0:
                 cond = cond_sample.unsqueeze(1).to(self.device)
            # Need to modify to srn cond
