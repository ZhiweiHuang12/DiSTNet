...
def calculate_kl(sample1,sample2):
    ...
    pmf1 += 1e-10
    pmf2 += 1e-10
    pmf1 = pmf1[:(int(np.max(sample1)+1))]/sum(pmf1[:(int(np.max(sample1)+1))])
    pmf2 = pmf2[:(int(np.max(sample1)+1))]/sum(pmf2[:(int(np.max(sample1)+1))])
...
def plot_jointplot(data1,data2,xy_ticks,ax,lims,label_names,fontsize = 6,cmap_name = "Blues",fill_flag=True,color="#d37166",data_nn=False,tick_width=0.4,labelsize=5):
    if data_nn:
        label_names = [el + " (DiSTNet)" for el in label_names]
    else:
    ...
    g0.ax_marg_x.tick_params(axis='x', which='major', labelsize=labelsize, length=length,width=tick_width)  # Set tick length for top histogram
    g0.ax_marg_y.tick_params(axis='y', which='major', labelsize=labelsize, length=length,width=tick_width)  # Set tick length for right histogram
    

    
    linewidth = 0.5
    for spine in g0.ax_joint.spines.values():
        spine.set_linewidth(linewidth)  # Set main plot border line width to 2
    for spine in g0.ax_marg_x.spines.values():
        spine.set_linewidth(linewidth)  # Set top marginal plot border line width
    for spine in g0.ax_marg_y.spines.values():
        spine.set_linewidth(linewidth)  # Set right marginal plot border line width
    if len(xy_ticks[0])>0:
