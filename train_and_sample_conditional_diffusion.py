...
# for dataset in ["SIRS","toggle_switch","center_dogma"]:
for dataset in ["afl","delay_SIRS","SIRS"]:
    ...
    if dataset == "mnist":
        ...
        latest_weight_index = 38
        PATH_TO_READY_MODEL = weight_dir + "model_{}.pth".format(latest_weight_index)
        ...
        data = get_afl_loader_and_transform(json_data = json_data,max_value=max_value)
        PATH_TO_READY_MODEL = weight_dir + "model_{}.pth".format(14)
        PATH_TO_READY_MODEL = None
    elif dataset=="toggle_switch":
        json_file = "/GPUFS/sysu_jjzhang_1/hzw/academicCode/MasterDPM/data/toggle_switch/(toggle_switch)_(4502).json" # 20001
        with open(json_file, 'r') as f:
            json_data = json.load(f)  # Parse JSON file
        max_li = []
        for i in range(len(json_data)):
        ...
        json_file = "/GPUFS/sysu_jjzhang_1/hzw/academicCode/MasterDPM/data/SIRS/(SIRS)_(2501).json" # 20001
        with open(json_file, 'r') as f:
            json_data = json.load(f)  # Parse JSON file
        max_li = []
        for i in range(len(json_data)):
        ...
        max_value = max(max_li)
        data = get_afl_loader_and_transform(json_data = json_data,max_value=max_value)
        latest_weight_index = 14
        PATH_TO_READY_MODEL = weight_dir + "model_{}.pth".format(latest_weight_index)
        # PATH_TO_READY_MODEL = None
    elif dataset=="delay_SIRS":
        data_index = 3004
        json_file = "/GPUFS/sysu_jjzhang_1/hzw/academicCode/MasterDPM/data/delay_SIRS/(delay_SIRS)_({}).json".format(data_index) # 20001
        with open(json_file, 'r') as f:
            json_data = json.load(f)  # Parse JSON file
        max_li = []
        for i in range(len(json_data)):
    ...
    if PATH_TO_READY_MODEL is not None:
        ...
        if os.path.exists(PATH_TO_READY_MODEL):
            ddpm.load_state_dict(torch.load(PATH_TO_READY_MODEL, map_location=device))
            print("continue train {}".format(PATH_TO_READY_MODEL))
        _, val_losses = train(
            model=ddpm,
